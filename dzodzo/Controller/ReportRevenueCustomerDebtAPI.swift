...
class ReportRevenueCustomerDebtAPI {

    static func getData(pstartdate: String, penddate: String, success: @escaping ([ReportRevenueCustomerDebt])-> Void) {
        ...
        let componentsGetPost = URLComponents(scheme: "https", host: "dzodzo.com.vn", path: "/a/api", queryItems: [
            ...
            URLQueryItem(name: "Service", value: "crud_search_report_customer_debt"),
        ...
        print("URL ReportRevenueCustomerDebt: \(urlGetPos)")
        ...
        Alamofire.request(urlRequest).responseJSON{ response in
            ...
            var model = [ReportRevenueCustomerDebt]()
            for dic in getposAPI {
                model.append(ReportRevenueCustomerDebt(dic))
            }
            success(model)
            print("DATA: \(model)")
