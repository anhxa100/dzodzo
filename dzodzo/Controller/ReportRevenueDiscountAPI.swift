//
//  ReportRevenueDiscountAPI.swift
//  dzodzo
//
...
class ReportRevenueDiscountAPI {
    
    static func getData(pstartdate: String, penddate: String, success: @escaping ([ReportRevenueDiscount])-> Void) {
        ...
        let componentsGetPost = URLComponents(scheme: "https", host: "dzodzo.com.vn", path: "/a/api", queryItems: [
            ...
            URLQueryItem(name: "Service", value: "crud_search_report_discount_service"),
        ...
        print("URL ReportRevenueDiscount: \(urlGetPos)")
        ...
        Alamofire.request(urlRequest).responseJSON{ response in
            ...
            var model = [ReportRevenueDiscount]()
            for dic in getposAPI {
                model.append(ReportRevenueDiscount(dic))
            }
            success(model)
            print("DATA: \(model)")
